...
***** /type/
      ...
      If type of field is `group`, then field entry actually
      describes a named group of fields, contents of which must be
      given in group fields file (see below).
...
**** Group fields
     ...
     ("resources/field-groups.json"), which must contain a list of
     JSON objects with keys `groupName` (which sets the name of
     group`) and `fields`, which is a list of field objects using the
     same syntax as model description. There's no way to set
     permissions for complex fields currently.
     ...
     Using `complex` field type in group field description is not
