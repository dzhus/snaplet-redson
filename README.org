...
***** /indexCollate/
      If field has index=true, then indexCollate instruct Redson
      whether or not to perform string cleaning (removing punctuation,
      space, converting to lower-case) prior to saving index values
      and performing an ad-hoc search against them. Defaults to false.
      Set this to true if you want to search by names or other
