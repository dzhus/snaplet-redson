...
** CRUD
   ...
   top-level snaplet under `/rs` URL).

   `/rs/<formid>/` serves title page (which is in fact the same for
   any <formid>) with all the code required to process metamodels on
   client side. Client JavaScript further requests metamodel
   description in bare JSON using `/rs/<formid>/model/` URL, from
...
**** CREATE
     ...
     : curl localhost:8000/rs/scp/ -X POST -d "{\"title\":\"Able\", \"code\":\"076\", \"class\":\"Keter\"}"
...
**** READ
     ...
     : curl localhost:8000/rs/scp/24/ -X GET
...
**** UPDATE
     ...
     : curl localhost:8000/rs/scp/24/ -X PUT -d "{\"title\":\"Able\", \"code\":\"076-2\", \"class\":\"Keter\", \"description\":\"Really nasty thing\"}"
...
**** DELETE
     ...
     : curl localhost:8000/rs/scp/24/ -X DELETE
...
*** Timeline
    ...
    `/rs/scp/timeline/` serves JSON list of last N (currently 10)
    timeline items for metamodel "scp":

    : curl localhost:8000/rs/scp/timeline/ -X GET
...
*** WebSockets notifications
    ...
    `/rs/<modelname>/events/` provides instance creation/deletion
...
** BLOCKED Routing bug
   ...
   `/rs/<formname>/` requests to emptyPage handler by checking `id`
...
** TODO Access control
   Per-field permissions (r/w) for different user groups (roles).
